...
class main:
    ...
    def pencil_draw(self, event=None):
        if self.left_button == "down":
            if self.x_position is not None and self.y_position is not None:
                self.c.create_line(self.x_position, self.y_position, event.x, event.y, smooth=True, fill=self.color_fg,
                                   width=self.penwidth)

    def line_draw(self, event=None):
        if None not in (self.x1_line_pt, self.x2_line_pt, self.y1_line_pt, self.y2_line_pt):
            self.c.create_line(self.x1_line_pt, self.x2_line_pt, self.y1_line_pt, self.y2_line_pt, smooth=True,
                               fill=self.color_fg, width=self.penwidth)
    ...
    def oval_draw(self, event=None):
        if None not in (self.x1_line_pt, self.x2_line_pt, self.y1_line_pt, self.y2_line_pt):
            self.c.create_oval(self.x1_line_pt, self.x2_line_pt, self.y1_line_pt, self.y2_line_pt,
                               outline=self.color_fg, width=self.penwidth)

    def rect_draw(self, event=None):
        if None not in (self.x1_line_pt, self.x2_line_pt, self.y1_line_pt, self.y2_line_pt):
            self.c.create_rectangle(self.x1_line_pt, self.x2_line_pt, self.y1_line_pt, self.y2_line_pt,
                                    outline=self.color_fg, width=self.penwidth)
